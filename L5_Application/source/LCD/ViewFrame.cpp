...
ViewFrame::ViewFrame(Screen &screen, Point size, Point position) :_size(size), _position(position), _screen(&screen),_maskSize(SCREEN_WIDTH,SCREEN_HEIGHT),_maskPosition()  {
    setCorners();
    setMaskCorners();
}
